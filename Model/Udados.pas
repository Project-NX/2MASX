...
uses
  ...
  IdTCPClient, IdExplicitTLSClientServerBase, IdFTP, Vcl.AppEvnts;
...
type
  ...
  TDados = class(TDataModule)
    ...
    qryItensCadCOUNT: TIntegerField;
    qryItensNegativo: TFDQuery;
    qryItensNegativoCOUNT: TIntegerField;
...
begin
  ...
  begin
    ...
    begin
      if not(qryPermissoes.locate('FKTELA',
        qryTelas.FieldByName('CODIGO').Value, [])) then
...
begin
  try
    ...
    if
      (qryParametroSERVIDOR_APP.AsString = EmptyStr) or
      ...
      (dados.qryParametroSENHA_APP.AsString = EmptyStr)
    then
      begin
        ...
        dados.qryParametroDATABASE_LI.AsString:=  dados.crypt('C', 'Nome do banco de dados MySql');
        dados.qryParametroUSUARIO_LI.AsString :=  dados.crypt('C', 'Usuário do banco de dados MySql');
        dados.qryParametroSENHA_LI.AsString   :=  dados.crypt('C', 'Senha do banco');

        dados.qryParametroDATABASE_APP.AsString :=  dados.crypt('C', 'Nome do banco de dados MySql APP');
        dados.qryParametroUSUARIO_APP.AsString  :=  dados.crypt('C', 'Usuário do banco de dados MySql APP');
        dados.qryParametroSENHA_APP.AsString    :=  dados.crypt('C', 'Senha do Banco APP');
  ...
  try
    ...
    Except
      ShowMessage('Não foi possivel conectar na base de dados!' + sLineBreak +
        'Verifique e pasta C:\Sistema\Dados\DADOS.FDB' +
