...
begin
  ...
  try
    try
      ...
      begin
        ShowMessage('vou assinar');
        DmNFe.ACBrNFe.NotasFiscais.Assinar;

        if not(qryVenda.State in dsEditModes) then
          qryVenda.Edit;

        qryVendaCHAVE.Value    :=  copy(DmNFe.ACBrNFe.NotasFiscais.Items[0].NFe.infNFe.Id, 4, 100);
        ...
        if Dados.qryconsulta.FieldByName('IE').IsNull then
          ...
          NotaF.NFe.Transp.Transporta.IE := Dados.qryconsulta.FieldByName
            ('IE').AsString;
                ...
                NotaF.NFe.Transp.Transporta.IE := '' ; // MARCIO
...
begin
  ...
  Dados.qryconsulta.Open;
  try
    NotaF.NFe.Dest.CNPJCPF := TiraPontos(Dados.qryconsulta.FieldByName('CNPJ').Value);
    NotaF.NFe.Dest.ISUF    := '';
    ...
    case StrToIntDef(Dados.qryconsulta.FieldByName('ISENTO').Value, 3) of
      0:
        begin
          ...
          NotaF.NFe.Dest.IE        := TiraPontos(Dados.qryconsulta.FieldByName('IE').Value);
        end;
      1:
        begin
    ...
    then
      NotaF.NFe.Dest.EnderDest.fone := TiraPontos(Dados.qryconsulta.FieldByName('FONE1').AsString)
    ...
    then
      ...
      <> '' then
      NotaF.NFe.Dest.EnderDest.fone  :=  TiraPontos(Dados.qryconsulta.FieldByName('CELULAR2').AsString)
    else
      NotaF.NFe.Dest.EnderDest.fone  :=  TiraPontos(Dados.qryconsulta.FieldByName('CELULAR2').AsString);

    NotaF.NFe.Dest.EnderDest.CEP     :=  StrToInt(TiraPontos(Dados.qryconsulta.FieldByName('CEP').AsString));
    NotaF.NFe.Dest.EnderDest.xLgr    := Dados.qryconsulta.FieldByName('ENDERECO').Value;
    NotaF.NFe.Dest.EnderDest.nro     := Dados.qryconsulta.FieldByName('NUMERO').Value;
    NotaF.NFe.Dest.EnderDest.xCpl    := '';
    NotaF.NFe.Dest.EnderDest.xBairro := Dados.qryconsulta.FieldByName('BAIRRO').Value;
    NotaF.NFe.Dest.EnderDest.cMun    := Dados.qryconsulta.FieldByName('CODMUN').Value;
    NotaF.NFe.Dest.EnderDest.xMun    := Dados.qryconsulta.FieldByName('MUNICIPIO').Value;
