...
uses
  ...
  Vcl.Menus, dxSkinsCore, dxSkinDevExpressDarkStyle, dxSkinOffice2019Colorful,
...
uses Udados, uProdutos, uGrupo, uUnidade, uTabelaIcms, uMarca;
...
begin
  if Length(DBEdit40.Text) >= 4 then
    begin
      qryCEST.Close;
      qryCEST.Params[0].Value :=
        Copy(DBEdit40.Text, 1, 3) + '%';
      qryCEST.Open;
      qryCEST.First;
...
begin
  ...
  if Dados.qryProdImpPadrao.Locate('SIGLA', Tipo, []) then
    begin
      ...
      qryProdutosCFOP_EXTERNO.Value := StrToIntDef(Dados.qryProdImpPadraoCFOP_FORA.Value, 6202);
      ...
      qryProdutosALIQ_ICM.AsFloat := Dados.qryProdImpPadraoALIQ_ICMS_P_DENTRO.AsFloat;
      qryProdutosALIQ_ICMS_EXTERNO.AsFloat := Dados.qryProdImpPadraoALIQ_ICMS_P_FORA.AsFloat;
